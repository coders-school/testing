...
Interpreter::extractRecord(std::string input) {
...
std::vector<int> Interpreter::processLaneRecordToRolls(std::string input) {
...
std::string Interpreter::getGameStatus(std::string lane) {
  if (lane.empty())
    return "no game";
  if (isFinished(lane))
...
bool Interpreter::isFinished(std::string lane) {
...
//  return isSingleGamerFinished(lane);
}
